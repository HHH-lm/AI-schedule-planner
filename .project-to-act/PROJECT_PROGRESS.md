...
| T-027 v0.1.0 首次发布执行（阶段 7 发布清单 + G7-001 发布 Gate） | 进行中 | Codex（用户指令）；发布确认人：项目负责人 | 发布清单 A-I 全部完成 + G7-001 记录 + 证据入账 | E-T027-001（部分执行） | 2026-08-17 |
| T-029 修复提醒时间已过 10 分钟仍推送（F-018 修订） | 已完成 | Codex（用户反馈定位） | 后端 pytest 全通过 + collect_due_blocks 边界测试 | E-T029-001 | 2026-08-18 |
...
- 2026-08-18：修复提醒过期推送（F-018 修订，T-029）：`collect_due_blocks` 仅接收最近 10 分钟内的到期 `remindAt`，提醒时间已过去超过 10 分钟（含服务恢复后积压）不再进入 due；同步更新提醒/可观测性测试与 README；后端 pytest 171 通过。证据 ID：E-T029-001。遗留问题：无。下一步：用户实测确认过期时间块不再收到微信提醒。来源：用户反馈定位。
- 2026-08-17：跨天时间块支持（F-022）实现：TimeBlock.end 支持超过 1440 的分钟偏移，BlockModal 增加结束日期，NLP/AI 支持跨天表达，周计划按天分段渲染，冲突/槽位/今日待办/任务看板/统计周报/提醒/记忆分析/validator 同步跨天语义；golden set 扩至 33 条并纳入跨天/1440 边界；后端 pytest 170、前端 Vitest 91、tsc、lint、scan 5/5、生产构建全部通过；真实 DeepSeek golden 33/33、`passed=true`；浏览器 DOM 验收 NLP 跨天生成两段渲染。证据 ID：E-F022-001。遗留问题：拖拽与跨周渲染需人工确认。下一步：人工验收拖拽/跨周后更新 F-022 状态并决定是否入 T-027 发布范围。来源：用户指令。
- 2026-08-17：T-027 发布清单执行（自动化部分）：A 组质量门禁复跑全部通过，版本号一致，公网前后端健康 200、cron 未授权 401，schema/reminder_log/Auth/RLS/DeepSeek/推送 API 复用 E-T024-002 与 E-G6-001；清单已勾选自动化项，C/G/H/I 与手机实际收到、Vercel Logs 等人工项保持未勾选。证据 E-T027-001。遗留问题：项目负责人需逐项完成人工确认后执行 G7-001。下一步：人工确认后追加版本历史、打 tag v0.1.0 并落账 G7-001。来源：用户指令。
