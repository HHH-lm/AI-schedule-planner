...
def test_golden_set_has_30_cases_with_expected_distribution() -> None:
    assert len(GOLDEN_AI_CASES) == 30
    ids = [case["id"] for case in GOLDEN_AI_CASES]
    assert len(set(ids)) == 30
    counts = Counter(case["kind"] for case in GOLDEN_AI_CASES)
    assert counts["quickadd"] == 10
        ...
        if case["kind"] in ("quickadd", "boundary"):
            ...
            if case.get("expect_reject"):
                ...
                for item in schedules:
                    ...
                    assert 0 <= item["start"] < item["end"] <= 1440
